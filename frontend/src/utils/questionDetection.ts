...
/**
 * 获取当前生效的系统提示词（自定义优先，无则返回null由调用方走默认）
 */
export const getCustomSystemPrompt = (): string | null => {
  const v = localStorage.getItem(CUSTOM_PROMPT_KEY)
 ...
 * 保存自定义系统提示词
 */
export const setCustomSystemPrompt = (text: string) => {
 ...
 * 重置为默认系统提示词
 */
export const resetCustomSystemPrompt = () => {
  ...
  const stablePrefix = [
    {
      ...
      content: getCustomSystemPrompt() || STABLE_SYSTEM_PROMPT
    }
  ]
